...
impl Image {
    ...
    pub fn embed_message(&mut self, message: &str) -> Result<(), JsValue> {
        let mut dct_blocks = self.to_dct_blocks().map_err(|e| JsValue::from_str(&e.to_string()))?;
        
        let mut message_bits = message.bytes().flat_map(|byte| (0..8).map(move |i| (byte >> i) & 1));
        
        for block in dct_blocks.iter_mut() {
            if let Some(bit) = message_bits.next() {
                // Modify the DC coefficient based on the bit
                block[0] = if bit == 1 { block[0].ceil() } else { block[0].floor() };
            } else {
                break;
            }
        }
        
        self.from_dct_blocks(dct_blocks).map_err(|e| JsValue::from_str(&e.to_string()))
    }
    ...
    pub fn extract_message(&self) -> Result<String, JsValue> {
        let dct_blocks = self.to_dct_blocks().map_err(|e| JsValue::from_str(&e.to_string()))?;
...
impl Dct for Image {
    ...
    fn quantize(&self, dct: Vec<f64>) -> Vec<f64> {
        dct.iter().zip(QUANTIZATION_TABLE.iter())
    ...
    fn unquantize(&self, quantized: Vec<f64>) -> Vec<f64> {
        quantized.iter().zip(QUANTIZATION_TABLE.iter())
            .map(|(&q, &t)| q * t as f64)
            .collect()
    }
    ...
    fn inverse_dct(&self, dct: &[f64]) -> Vec<u8> {
        ...
        block.into_iter().map(|p| p.round().clamp(0.0, 255.0) as u8).collect()
    }
}
